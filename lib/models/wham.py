...
class Network(nn.Module):
    ...
    def forward(self, x, inits, img_features=None, mask=None, init_root=None, cam_angvel=None,
                cam_intrinsics=None, bbox=None, res=None, return_y_up=False, refine_traj=True, states=None, **kwargs):
        ...
        m_state, t_state, md_state = states if states is not None else (None, None, None)
        pred_kp3d, motion_context, m_state = self.motion_encoder(x, init_kp, state=m_state)
        self.old_motion_context = motion_context.detach().clone()
        
        # Stage 2. Decode global trajectory
        pred_root, pred_vel, t_state = self.trajectory_decoder(motion_context, init_root, cam_angvel, state=t_state)
        ...
        pred_pose, pred_shape, pred_cam, pred_contact, md_state = self.motion_decoder(motion_context, init_smpl, state=md_state)
        # --------- #
